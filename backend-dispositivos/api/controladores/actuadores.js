...
exports.listarActuadores = async function (req, res, next) {
    try {
        var datos = await r.db('iot').table('actuadores').run(r.conn);
...
exports.obtenerActuador = async function (req, res, next) {
    ...
    try {
        var resultado = await r.db('iot').table('actuadores').get(id).run(r.conn);
        res.status(200).json(resultado);
    } catch (err) {
        throw new Error(err);
        // analizar console.warn(err);
...
exports.eliminarActuador = async function (req, res, next) {
    ...
    try {
        var resultado = await r.db('iot').table('actuadores').get(id).delete().run(r.conn);
        res.status(200).json(resultado);
    } catch (err) {
        throw new Error(err);
        res.status(400).json({ status: 400, message: err.message });
